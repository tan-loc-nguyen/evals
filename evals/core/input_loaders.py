"""
Input loaders for LLM evaluation configurations.

This module provides data classes and functions for loading and processing
configuration files, prompts, and expected outputs for evaluations.
"""
...
@dataclass
class Prompt:
    """Data class representing a prompt with system and user components."""
    system_prompt: str
    user_prompt: str
...
@dataclass
class DataSet:
    """Data class representing a dataset with prompts and user inputs."""
    prompt: Prompt
...
class ExpectedOutput:
    """Data class representing expected output for evaluation."""
...
@dataclass
class Criterion:
    """Data class representing an evaluation criterion."""
...
@dataclass
class RawConfig:
    """Data class representing raw configuration data from YAML files."""
    mode: Literal["reference_free", "ground_truth", "comparison"]
    structured: bool
    candidates: List[Dict[str, Any]]
    judge: Dict[str, Any]
    criteria: List[Criterion]
    prompt_file: str | List[str] | None
    expected_response_file: str | List[str] | None
    metadata: Dict[str, Any] | None


@dataclass
class InputData:
    """Data class representing processed input data for evaluation."""
    prompt: Prompt
    mode: Literal["reference_free", "ground_truth"]
    structured: bool
    candidates: List[Dict[str, Any]] | List[Any]
    judge: Dict[str, Any]
    criteria: List[Criterion]
    expected_response: List[ExpectedOutput] | None
    metadata: Dict[str, Any] | None
...
def get_prompt_data(prompt_yaml_path: str) -> Prompt:
    """Load and process prompt data from YAML file."""
...
def get_expected_response_data(expected_response_file: str) -> List[ExpectedOutput]:
    """Load expected response data from YAML file."""
    with open(expected_response_file, 'r', encoding='utf-8') as f:
        expected_response_data = yaml.safe_load(f)

    return [ExpectedOutput(**expected_response)
            for expected_response in expected_response_data['expected_responses']]
...
def load_config(config_yaml_path: str) -> RawConfig:
    """Load configuration from YAML file and convert to RawConfig object."""
...
def get_config_data(config_yaml_path: str) -> InputData:
    """Load and process complete configuration data into InputData object."""
    ...
    if config_data.prompt_file:
        ...
        elif isinstance(config_data.prompt_file, list):
            # TODO: Handle multiple prompts with the same config
            prompt_data = [get_prompt_data(prompt_file)
                           for prompt_file in config_data.prompt_file]
    else:
    ...
    if config_data.expected_response_file:
        if isinstance(config_data.expected_response_file, str):
            expected_response_data = get_expected_response_data(
                config_data.expected_response_file)
        elif isinstance(config_data.expected_response_file, list):
            # TODO: Handle multiple expected outputs with the same config
            expected_response_data = [
                get_expected_response_data(expected_response_file)
                for expected_response_file in config_data.expected_response_file
            ]
    else:
